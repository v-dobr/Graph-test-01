...
Isso fornece a você um projeto para Android com uma atividade e um botão que você pode usar para autenticar o usuário.
...
### Adicionar a dependência a app/build.gradle
...
2. Localize o manipulador de eventos do evento de clique do *FloatingActionButton*. Substitua o método **onClick** pelo seguinte código. Insira a **ID do aplicativo** do seu aplicativo no espaço reservado marcado com **\<ID_DO_SEU_APLICATIVO\>**.
    ```java
    @Override
...
1. Você precisa informar ao sistema do Android que o aplicativo pode lidar com solicitações para *https://login.microsoftonline.com/common/oauth2/nativeclient*. Para fazer isso, abra o arquivo **AndroidManifest** e adicione o elemento de atividade seguinte.
    ```xml
...
### Chamar o Microsoft Graph usando o SDK do Microsoft Graph
...
3. Substitua a linha `String accessToken = tokenResponse.accessToken;` pelo seguinte código. Insira seu endereço de email no espaço reservado marcado com **\<SEU_ENDEREÇO_DE_EMAIL\>**.
    ```java
...
### Chamar o Microsoft Graph usando a API REST do Microsoft Graph
A [API REST do Microsoft Graph](http://graph.microsoft.io/docs) expõe várias APIs dos serviços de nuvem da Microsoft por meio de um único ponto de extremidade da API REST. Siga estas etapas para usar a API REST.
...
Verifique a caixa de entrada do endereço de email que você configurou em [Chamar Microsoft Graph](#call-the-microsoft-graph). É provável que você tenha um email da conta que utilizou para entrar no aplicativo.
